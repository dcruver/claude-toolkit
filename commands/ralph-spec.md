...
**Report**: item count (split by type: normal vs `[mvn]`), a one-line summary of what's in the Design Reference, and the exact command to run it (`ralph --plan PLAN.md`, pointing to `ralph --help` for `--max-attempts`/`--items`/`--review-gate`/`--model`).
