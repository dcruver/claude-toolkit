...
- [ ] Add a --help output to bin/okf listing every subcommand in SPEC.md §7 with its flags, printed for both --help and a bare invocation with no subcommand (see SPEC.md's Design Reference). Verify: ./tests/toolkit.sh
...
- [ ] Implement okf verify in bin/okf to append a verified entry carrying the --by actor, defaulting to human: plus the current username, and the current UTC timestamp in ISO 8601, preserving all unknown frontmatter keys (see SPEC.md's Design Reference). Verify: ./tests/toolkit.sh
- [ ] Extend okf verify in bin/okf to print the resulting SPEC.md §8 trust tier, covering its degradation to Machine-confirmed when the new entry predates generated.at or the concept is drifted (see SPEC.md's Design Reference). Verify: ./tests/toolkit.sh
